...
public class AlarmReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ObjPick objPick = new SessionManager(context).getSleep();
        if (objPick != null) {
            ...
            if ((timeHour == objPick.getTimeHour()) && (timeMinute == objPick.getTimeMinute()) && (date == creatTime.get(Calendar.DATE))) {
                Intent launchIntent = new Intent(context, ExperimentActivity.class);
                launchIntent.putExtra("VEVENING", true);
